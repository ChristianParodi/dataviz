...
function radarmeanChart() {
  d3.csv("./../../dataset/assignment_4/min_max_avg_states.csv", d3.autoType)
    .then(data => {
      const toggleUnit = d3.select("#toggle-unit-randmean");
      ...
      function drawChart(selectedState, isFahrenheit) {
        d3.select("#randmean-chart").select("svg").remove();
        ...
        const svg = d3.select("#randmean-chart").append("svg")
          .attr("width", width)
          .attr("height", height)
          .append("g")
        ...
        aggregatedData.forEach((yearData, index) => {
          ...
          const pathData = months.map(month => {
            const monthData = yearData.values.find(v => v.month === month);
            const angle = angleScale(month);
          ...
          svg.selectAll(`.circle-year-${index}`)
            .data(yearData.values)
            .join("circle")
            .attr("class", `circle-year-${index}`)
            .attr("cx", d => {
            ...
            .on("mouseover", function (event, d) {
              tooltip.style("opacity", 1)
                ...
                .html(`Year: <strong>${yearData.year}</strong><br>Month: <strong>${monthsMap[d.month]}</strong><br>Average Temp: <strong>${convert(d.avg).toFixed(1)}${isFahrenheit ? '°F' : '°C'}</strong>`);
            })
      ...
      toggleUnit.on("change", function () {
        drawChart(firstState, this.checked);
      });

      const stateSelector = d3.select("#state-selector-randmean");
